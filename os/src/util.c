...
// Computes the power of a given intger base to the given *unsigned* intereger exponent in log(n) time
...
int __itoa(void *target, size_t (* writer)(void *, const char *), int i, int base) {
  ...
  int wr = 1;
  for(exp = pow(base, digits-1); digits && wr; --digits, i %= exp, exp /= base, wrote += wr) {
...
size_t __printf(void *target, size_t (* writer)(void *, const char *), const char *fmt, varg_type varg){
